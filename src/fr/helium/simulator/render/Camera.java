...
public class Camera {
    ...
    public void moveFromLook(float dx, float dy, float dz) {
    	...
    	Vector right = forward.cross(Vector.UP).normalize();
    	location = location.add(forward.mul(dz));
    	location = location.add(Vector.UP.mul(dy));
    	location = location.add(right.mul(-dx));
    }
